...
logFile <- read.delim(comment.char = "#", file = fileName) %>% as_tibble()
ESSthresh <- 200

stateSize <- logFile$state[3] - logFile$state[2]
guessSize <- logFile$state[nrow(logFile)] * 0.1 / stateSize

burnInPeriods <- apply(logFile %>% filter(state > 1) %>% select(-state), MARGIN = 2, function(x){kneedle(x, guess = guessSize) * stateSize})
...
tracePlot <- logFile %>% filter(state > 10000) %>% pivot_longer(-state, names_to = "Statistic", values_to = "Value") %>% ggplot(aes(x = state, y = Value)) +
	...
	geom_vline(xintercept = burnInPeriods["joint"], colour = "red", lty = 2) +
	theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1))

ggsave(tracePlot, file = gsub(".log","TracePlot.png", fileName), width = 24, height = 18)

summaryLog <- logFile %>% pivot_longer(-state, names_to = "Statistic", values_to = "Value") %>%
       	mutate(state = replace(state, state < burnInPeriods["joint"],NA)) %>%
       	filter(!is.na(state)) %>% group_by(Statistic) %>%
       	summarize(Mean = mean(Value), Median = median(Value), SE = qnorm(0.975) * sd(Value)/sqrt(length(Value)),
	CIHi = Mean + SE, CILo = Mean - SE)

probPlot <- logFile %>% pivot_longer(-state, names_to = "Statistic", values_to = "Value") %>%
       	mutate(state = replace(state, state < burnInPeriods["joint"],NA)) %>%
